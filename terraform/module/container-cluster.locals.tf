locals {
  container_clusters = [
    for cluster in var.container_clusters : {
      ...
      initial_node_count       = cluster.initial_node_count != null ? cluster.initial_node_count : var.default_container_cluster_default_pool_initial_node_count
      container_node_pools = cluster.container_node_pools == null ? [] : [
        for pool in cluster.container_node_pools : {
          ...
          location   = pool.location != null ? cluster.location : var.default_region
          node_count = pool.node_count != null ? pool.node_count : var.default_container_cluster_node_pool_initial_node_count
          node_config = pool.node_config == null ? {
            preemptible            = null
            machine_type           = null
            service_account        = null
            create_service_account = var.default_container_cluster_node_pool_create_service_account
            oauth_scopes           = null
            } : {
            ...
            create_service_account = (
              pool.node_config.create_service_account != null
              ? pool.node_config.create_service_account
              : var.default_container_cluster_node_pool_create_service_account
            )
      ...
      for pool in cluster.container_node_pools : {
        ...
        node_config = {
          ...
          service_account = (
            ...
            : pool.node_config.create_service_account == true
            ? "gke-pool-${pool.name}-sa@${pool.project}.iam.gserviceaccount.com"
            : null
          )
          create_service_account = pool.node_config.create_service_account
          oauth_scopes           = pool.node_config.oauth_scopes
        }
      }
    ]
  ...
  container_node_pool_service_accounts = [
    for pool in local.container_node_pools : {
      pool_name    = pool.name
      project      = pool.project
      account_id   = "gke-pool-${pool.name}-sa"
      display_name = "Node Pool ${pool.name} Service Account"
    } if pool.node_config.create_service_account == true
  ]
}
